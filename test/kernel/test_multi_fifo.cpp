 ...
 *
 * test_multi_fifo: benchmark two-sided AlltoAll with varying interContextCount
 * (number of FIFO contexts). Runs with 1 context first, then 4, 8, 16, and
 * FLAGCX_DEVICE_CTA_COUNT, printing latency and bandwidth for each.
 ************************************************************************/
...
static bool runAlltoAll(flagcxComm_t comm, flagcxDeviceHandle_t devHandle,
  ...
  FLAGCXCHECK(flagcxDevMemCreate(comm, sendBuff, maxBytes, nullptr, &sendMem));
  FLAGCXCHECK(flagcxDevMemCreate(comm, recvBuff, maxBytes, nullptr, &recvMem));
...
int main(int argc, char *argv[]) {
  ...
  // Ensure enough data for all CTAs to get work: 36 CTAs × 32 threads each
  ...
  if (proc == 0) {
    printf("\n# Multi-FIFO benchmark: two-sided AlltoAll, %d ranks\n",
           totalProcs);
  ...
  for (int ci = 0; ci < nConfigs; ci++) {
    ...
    bool pass =
        runAlltoAll(comm, devHandle, stream, sendBuff, recvBuff, hello,
                    minBytes, maxBytes, stepFactor, numWarmupIters, numIters,
                    totalProcs, proc, worldSize, ctxCounts[ci]);
    if (!pass)
      anyFail = true;
    MPI_Barrier(MPI_COMM_WORLD);
  }
