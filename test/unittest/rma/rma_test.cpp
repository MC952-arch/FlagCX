...
void RmaTest::SetUpTestSuite() {
  ...
  // flagcxGet only requires the registered data window. Keep its coverage
  // independent from stream-ordered signal support.
  ...
  if (res != flagcxSuccess || signalBuff == nullptr) {
    ...
    FAIL() << signalRmaSkipReason << " (result=" << res << ")";
  }
