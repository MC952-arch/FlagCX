...
nvshmemDevCommCreate(flagcxComm_t comm,
  ...
  sc->worldBarrierCount = reqs->intraBarrierCount;
  ...
  if (sc->counterCount > 0) {
    cudaMalloc(&sc->counterBuffer, sc->counterCount * sizeof(uint64_t));
    cudaMemset(sc->counterBuffer, 0, sc->counterCount * sizeof(uint64_t));
  }

  // Shadow buffer (local device memory)
  if (sc->signalCount > 0) {
    cudaMalloc(&sc->shadowBuffer, sc->signalCount * sizeof(uint64_t));
  ...
  if (totalBarriers > 0) {
    cudaMalloc(&sc->barrierUsage, totalBarriers * sizeof(uint64_t));
  ...
  nvshmem_team_split_strided(NVSHMEM_TEAM_WORLD, nodeId * sc->intraSize, 1,
                             sc->intraSize, NULL, 0, &sc->intraTeam);

  // Team creation: inter-node
  if (interSize > 1) {
    nvshmem_team_split_strided(NVSHMEM_TEAM_WORLD, sc->intraRank, sc->intraSize,
                               interSize, NULL, 0, &sc->interTeam);
  } else {
