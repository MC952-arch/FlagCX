...
struct FlagcxP2pEngine {
  ...
  std::shared_mutex connectionLifetimeMutex;
...
struct FlagcxP2pConn {
  ...
  mutable std::shared_mutex remoteRegionsMutex;
...
struct FlagcxP2pNetTask {
  ...
  FlagcxP2pNetTask() : conn(NULL), nextSlice(0), done(false), failed(false) {}
};
...
static bool findRemoteMrChunk(const FlagcxP2pConn *conn, uint64_t addr,
  ...
  if (conn == NULL || out == NULL)
    return false;
  std::shared_lock<std::shared_mutex> lock(conn->remoteRegionsMutex);
...
static void updateRemoteRegion(FlagcxP2pConn *conn, FlagcxP2pNotifType type,
                               const FlagcxP2pMrCtrlPayload &payload) {
  std::unique_lock<std::shared_mutex> lock(conn->remoteRegionsMutex);
...
static int bootstrapExchangeDescTable(struct bootstrapState *bsState,
  ...
  std::unique_lock<std::shared_mutex> regionsLock(conn->remoteRegionsMutex);
...
void flagcxP2pEngineConnDestroy(FlagcxP2pConn *conn) {
  ...
  if (conn->engine != NULL) {
    std::unique_lock<std::shared_mutex> lifetimeLock(
        conn->engine->connectionLifetimeMutex);
    {
      std::lock_guard<std::mutex> connectionLock(conn->engine->connectionMutex);
      conn->mrPublishReady = false;
      conn->engine->connections.erase(conn);
    }
...
int flagcxP2pEngineRegEx(FlagcxP2pEngine *engine, uintptr_t data, size_t size,
  ...
  if (!flagcxParamMrSortedLookup()) {
    ...
    if (!publishMrAdd(engine, registration, entry.mrId)) {
      ...
      publishMrRemove(engine, entry.mrId);
      gMemRegInfo.erase(data);
      gMrToBaseAddr.erase(entry.mrId);
  ...
  flagcxResult_t res = flagcxMrRegistryRegister(
      ...
      registration, p2pExt, &assignedId);
  ...
  if (!publishMrAdd(engine, registration, assignedId)) {
    ...
    publishMrRemove(engine, assignedId);
    void *removedExt = NULL;
    flagcxMrRegistryDeregister(flagcxGlobalMrRegistry, data,
...
submitNetTransfer(FlagcxP2pConn *conn,
  ...
  if (conn->engine->scheduler == NULL)
    return -1;
  return addNetTransfer(conn, conn->engine->scheduler->submit(conn, planned),
                        transferId);
}
