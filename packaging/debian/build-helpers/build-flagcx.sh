...
if [ -z "$BACKEND" ]; then
    ...
    echo "  base_image_version - Base image version tag (default: 2.1.2)"
    echo ""
    echo "Examples:"
    echo "  $0 nvidia"
    echo "  $0 metax"
...
case "$BACKEND" in
    nvidia)
        BASE_IMAGE="harbor.baai.ac.cn/flagos-base/flagos-base-nvidia-cuda12.8"
        VENDOR="nvidia"
        ;;
    metax)
...
# Extract .deb files
CONTAINER_NAME="flagcx-deb-${BACKEND}-tmp-$$"
